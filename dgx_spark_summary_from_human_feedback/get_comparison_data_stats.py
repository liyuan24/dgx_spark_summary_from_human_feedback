...
if __name__ == "__main__":
    ...
    validation_dataset = validation_dataset.filter(lambda x: x['batch'] not in cnn_batches)
    tokenizer = AutoTokenizer.from_pretrained("Qwen/Qwen2.5-0.5B")

    train_response_lengths = get_reference_response_lengths(train_dataset, tokenizer)
    validation_response_lengths = get_reference_response_lengths(validation_dataset, tokenizer)
